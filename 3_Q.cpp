...
public:
    vector<Weight> Dijkstra(Index from, Index to = -1) const {
        vector<Weight> distance(master_.Size(), kWeightMax);
        vector<Weight> opt(master_.Size(), kWeightMax);
        ...
        while (!que.empty()) {
            ...
            for (; next != master_.GetNeighbors(current).end(); ++next, ++weight) {
                if (distance[*next] == kWeightMax && opt[*next] > time + *weight) {
                    que.insert({time + *weight, *next});
                    opt[*next] = time + *weight;
                }
            }
        }
...
void Work() {
    ...
    size_t out = 0;
    size_t count = 0;
    cin >> dest >> up >> down >> in >> out >> count;
    graph::GraphAdjacentWeighted graph_what(height + count);

    std::set<size_t> important;
    important.insert(0);
    important.insert(dest - 1);
    ...
    for (size_t iter = 0; iter < count; ++iter) {
        ...
        while (complexity--) {
            size_t place = 0;
            cin >> place;
            graph_what.Link(place - 1, height + iter, in);
            graph_what.Link(height + iter, place - 1, out);
            important.insert(place - 1);
        }
    }

    size_t previous = 0;
    for (auto link : important) {
        if (previous != link) {
            graph_what.Link(previous, link, up * (link - previous));
            graph_what.Link(link, previous, down * (link - previous));
        }
        previous = link;
    }

    graph::LightestWay way(graph_what);
    auto result = way.Dijkstra(0, dest - 1);
    auto dist = result[dest - 1];
